...
class FileManager:
    ...
    def export_to_txt(filepath, title, body, translations):
        ...
        try:
            with open(filepath, 'w', encoding='utf-8') as f:
                ...
                # Store translations as a hidden-ish JSON string at the bottom
                f.write(json.dumps(translations, ensure_ascii=False))
            return True
    ...
    def export_to_csv(filepath, title, body, translations):
        ...
        try:
            with open(filepath, 'w', newline='', encoding='utf-8') as f:
                ...
                writer.writerow([title, body, json.dumps(translations, ensure_ascii=False)])
            return True
        except Exception as e:
    ...
    def import_from_file(filepath):
        """Reads a file and returns (title, body, translations_dict)."""
        try:
            if filepath.endswith('.csv'):
                with open(filepath, 'r', encoding='utf-8') as f:
                    reader = csv.DictReader(f)
                    row = next(reader)
                    return row['Title'], row['Main_Body'], json.loads(row['Translations_JSON'])
                ...
                with open(filepath, 'r', encoding='utf-8') as f:
                    ...
                    if "METADATA_TRANSLATIONS:" in content:
                        ...
                        # Body is everything between 'CONTENT:' and the separator line
